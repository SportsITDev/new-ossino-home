...
export const generateVipAffiliate = createAsyncThunk(
  ...
  async (_, { rejectWithValue, getState }) => {
    try {
      ...
      console.log('VIP Affiliate: Making API request for user', user.email);
      ...
      if (error?.response?.data?.result?.message === 'USER_NAME_ALREADY_EXIST') {
        console.log('VIP Affiliate: User already exists, checking for cached data and existing btag');
