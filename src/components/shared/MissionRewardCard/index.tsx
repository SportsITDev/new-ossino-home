  ...
  return (
    ...
    >
      <div className="relative w-[85px] flex justify-center pb-2">
        <div className="w-12 h-12 md:w-10 md:h-10 xl:h-12 xl:w-12 rounded-full bg-neon-1 flex flex-col justify-center items-center relative z-20">
          <span className="leading-3 md:text-xs md:leading-[12px] font-bold text-black text-center">
            {data.prizeAmount}
          </span>
          <span className="text-[10px] md:text-[9px] xl:text-[10px] leading-[10px] font-medium text-black">
            coins
          </span>
        </div>
        <img
      ...
      <span className={cn(classNames.description, 'text-white')}>Claimed!</span>
    </div>
  );
};
...
const IncompleteMissionReward = ({ data }: BaseMissionRewardProps) => {
  return (
    <div
      className={cn(
        'gap-2 md:gap-2 bg-gray-700 relative overflow-hidden xl:gap-2',
        classNames.wrapper,
      )}
    >
      {data.imageUrl ? (
      ...
      <div className="flex flex-col items-center gap-1">
        <span className={cn(classNames.label, 'text-gray-300')}>
          {data.description}
        </span>
        <div className="w-full bg-gray-600 rounded-full h-2">
          <div
            className="bg-neon-1 h-2 rounded-full transition-all duration-300"
            style={{ width: `${data.progress}%` }}
          />
        </div>
        <span className="text-xs text-gray-400">{data.progress}% Complete</span>
      </div>

      <div className="absolute bg-gradient-to-b from-5% from-[#011504] to-70% to-[#3F803C] w-[370px] h-[580px] rounded-full blur-[300px] rotate-[-150deg] bottom-[-500px] right-[-350px]" />
    </div>
  );
};
...
const AvailableMissionReward = ({
  ...
  return (
    <div
      ...
      className={cn(
        'gap-2.5 md:gap-2 bg-gradient-to-b from-[#34087B] to-80% to-[#5F0FE1] xl:gap-3.5',
        classNames.wrapper,
        {
