...
const PageHeader = ({
  ...
  const title = () => {
    const path = cleanUrlSegment(pathname);
  ...
  return (
    <div className={cn('flex justify-between mb-5', className)}>
      <div className="flex gap-2">
        ...
        {title() && (
          <div className="bg-gray-800 rounded-lg h-8 px-4 flex items-center w-fit">
            <span className="text-sm font-medium text-neon-2 capitalize">
              {title()}
            </span>
          </div>
        )}
      ...
      {children ? (
        <div
          className={cn(
            'bg-gray-800 rounded-lg h-8 px-4 flex items-center gap-2 w-fit',
            childrenClassName,
          )}
        >
          {children}
