...
const SortingColumn = ({ state, label, handleSort }: SortingColumnProps) => {
  ...
  return (
    <div className='flex items-center cursor-pointer' onClick={switchSortType}>
      {label && (
        <span className='text-xs font-medium leading-4 text-gray-300'>{label}</span>
      )}
      <div 
        className="sort-arrows relative h-4 w-4 flex justify-center items-center"
        
      >
        <Icon href={sortingArrowsIcon} id='sortingArrowsIcon' className="h-[7px] w-1 cursor-pointer" />
        {state === SORT_STATE.DESC && (
          <div className="absolute top-0 w-full h-1/2 bg-black" /> 
        )}
        {state === SORT_STATE.ASC && (
          <div className="absolute bottom-0 w-full h-1/2 bg-black" /> 
        )}
      </div>
    </div>
  );
