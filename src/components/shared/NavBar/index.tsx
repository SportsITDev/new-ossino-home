...
const Navbar = ({ isOpen, setIsOpen }: INavbarProps) => {
  ...
  const handleNavigation = (item: NavigationsItemsType) => {
    ...
    if (!isAuth && item.label !== 'Discovery' && item.label !== "Main") {
  ...
  return (
    <nav className="w-full md:w-[calc(100%-var(--scrollbar-width))] fixed h-[73px] bottom-0 left-0 xl:hidden z-50 bg-gray-800 flex items-center">
                ...
                <span className="flex items-center justify-center text-white">
                  <Icon
                    ...
                    className={cn('w-5 h-5 fill-current stroke-current', {
                      'text-neon-1 stroke-neon-1': selected(),
                    })}
                  />
                </span>
                <span
                  className={cn('text-xs text-white', {
                    'text-[#D8FF68]': selected(),
                  })}
                >
                  {item.label}
                </span>
