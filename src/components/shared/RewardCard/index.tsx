...
const DisabledReward = ({ data }: BaseRewardProps) => {
  return (
    <div
      className={cn(
        'gap-2.5 md:gap-2 bg-gray-700 relative overflow-hidden xl:gap-3.5',
        classNames.wrapper,
      )}
    >
      <img
    ...
    >
      <div className="relative w-[85px] flex justify-center pb-5">
        <div className="w-12 h-12 md:w-10 md:h-10 xl:h-12 xl:w-12 rounded-full bg-neon-1 flex flex-col justify-center items-center relative z-20">
          <span className="leading-4 md:text-sm md:leading-[14px] font-bold text-black">
            {data.value}
          </span>
          <span className="text-xs leading-[10px] md:text-[9px] md:leading-[9px] font-medium text-black">
            Coins
          </span>
        </div>
        <img
...
const AvailableReward = ({ onClick }: AvailableRewardProps) => {
  return (
    <div
      onClick={onClick}
      className={cn(
        'gap-2.5 md:gap-2 bg-gradient-to-b cursor-pointer from-[#34087B] to-80% to-[#5F0FE1] xl:gap-3.5',
        classNames.wrapper,
      )}
    >
      <img
