...
const UserCard = ({ currentsStatus, nextStatus, progress }: IUserCardProps) => {
  ...
  return (
    <Link
      to="/loyalty/rewards"
      className="flex flex-col items-center bg-gray-700 rounded-lg cursor-pointer no-underline"
    >
      ...
      <div className="border-b border-[#383838] w-[194px]" />
      <div className="flex w-full flex-col py-2 px-3">
        ...
        <div className="pt-2">
          ...
          <div className="flex w-full pt-1 justify-between">
            <span className="text-white text-xs">{roundedProgress}%</span>{' '}
            {nextStatus ? (
              ...
              <div className="flex items-center">
                <span className="text-white text-xs px-1">🏆 Max Level</span>
              </div>
            )}
          </div>
        </div>
