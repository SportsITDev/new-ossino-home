...
const UserProfileAvatar: FC<UserProfileAvatarProps> = ({
  ...
  return (
    <div className={`relative ${className}`}>
      <div className={`${sizeClasses[size]} bg-gray-600 rounded-lg flex items-center justify-center overflow-hidden`}>
        {isLoading ? (
      ...
      {withIndicator && (
        <div className={`absolute bottom-0 right-0 ${indicatorSizes[size]} bg-green-500 rounded-full border border-gray-800`} />
      )}
    </div>
  );
};
