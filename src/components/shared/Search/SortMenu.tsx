    ...
    <Table className={cn('min-w-[180px] xl:w-[250px] rounded-lg ')}>
      <TableBody className="flex flex-col px-4">
        {Object.entries(SORT_METHOD)
          ...
          .map(([key, value]) => (
            <TableRow key={key} className="border-b-[2px] border-b-[#383838] text-nowrap">
              <TableCell className="text-gray-200 w-full flex">
                <div
                  className={cn('w-full capitalize cursor-pointer', { 'text-neon-1': sort === value })}
                  onClick={(e) => {
                    e.stopPropagation()
