...
const Empty = () => {
  return (
    <div className="w-full rounded-xl bg-gray-800 h-[515px] flex items-center justify-center md:h-[444px] xl:bg-transparent xl:h-[600px]">
      <div className="flex flex-col items-center">
        ...
        />
        <p className="text-sm text-gray-400">No actions to show here</p>
      </div>
    </div>
  );
};
...
const LoadingState = ({ message }: { message: string }) => {
  return (
    <div className="w-full rounded-xl bg-gray-800 h-[515px] flex items-center justify-center">
      <div className="flex flex-col items-center">
        <div className="w-8 h-8 border-2 border-gray-400 border-t-white rounded-full animate-spin mb-4"></div>
...
const BonusesUsage = ({ bonuses }: { bonuses: RegisterBonus[] }) => {
  return (
...
const BonusesContent = () => {
  ...
  return (
    <Tabs defaultValue={currentTab} onValueChange={handleTabChange}>
      <TabsList className="border-b-gray-600 w-full justify-start">
      ...
      <TabsContent value="usage" className="mt-4">
        {usageLoading ? (
          ...
          <BonusesUsage bonuses={usageData} />
        ) : (
          <Empty />
        )}
      </TabsContent>
