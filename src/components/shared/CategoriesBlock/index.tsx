...
const CategoriesBlock = ({ applyCurrentCategory, currentCategory: propCurrentCategory }: ICategoriesBlockProps) => {
  ...
  // Fetch categories when selectedGameType changes
  useEffect(() => {
  ...
  const onClickHandler = (category: string) => {
    if (pathname === ROUTES.search) {
      // Use URL-based filtering for SearchPage to maintain state on refresh
      if (category === 'Lobby') {
      ...
      <Slider withShadow slidesPerView={xl ? 7 : undefined}>
        {Array.from({ length: 7 }).map((_, index) => (
          ...
          <SwiperSlide key={index} style={{ width: xl ? '173px' : 'auto' }}>
            <Skeleton className="h-9 rounded-[12px] bg-gray-600" />
          </SwiperSlide>
        ))}
      </Slider>
    );
  ...
  const selectedItem = (category: string) => {
    // For SearchPage, use URL-based highlighting
    if (pathname === ROUTES.search) {
      if (category === 'Lobby' && !categories.length) {
        return true;
      }
      // Use propCurrentCategory if provided, else fallback to URL categories
    ...
    // If currentCategory prop is provided (from other pages), use it
        ...
        return (
          <SwiperSlide key={item.id} style={{ width: xl ? '173px' : 'auto' }}>
            <Button
              ...
              className={cn(
                'bg-gray-750 w-full flex items-center justify-center gap-2 rounded-[12px] border border-transparent',
                { 'border-neon-2 ': selectedItem(item.label) },
              )}
            >
              {'id' in item.icon && (
                <Icon
                  id={item.icon.id}
                  href={item.icon.href}
                  className="w-5 h-5 fill-current text-[#80F31D]"
                />
              )}
              <span className={cn('text-white capitalize', { 'text-[#D8FF68]': selectedItem(item.label) })}>
