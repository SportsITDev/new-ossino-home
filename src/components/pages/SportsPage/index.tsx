  ...
  useEffect(() => {
    const handleMessageFromChild = (event: MessageEvent) => {
      ...
      try {
        ...
        console.log('Message received from child:', payload);

        if (payload.eventType === 'user-unauthorized') {
          console.log('Child requested logout → clear parent session');
          ...
          console.log('Iframe loaded:', payload.eventData.isLoaded);
          // setIsLoading(false);
          ...
          if (payload.eventData.isLoaded && accessToken && userId) {
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {betslipCount > 0 && (
        <button
          type="button"
          className="fixed left-1/2 -translate-x-1/2 z-50 flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-br from-black via-green-600 to-green-400 shadow-lg text-white text-3xl hover:scale-105 transition-transform md:hidden"
