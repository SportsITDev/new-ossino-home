...
const SearchPage = () => {
  ...
  const { search, sort, providers, categories: urlCategories } = useCustomQueryParams();

  // Initialize currentCategory from URL or default to 'Lobby'
  ...
  function getFilteredGames() {
    ...
    // If 'Lobby' is selected or no categories, show all games
    if (currentCategory.length === 0 || (currentCategory.length === 1 && currentCategory[0] === 'Lobby')) {
      ...
      // Filter by selected categories (support multiple)
  ...
  // Match the CSS grid breakpoints: grid-cols-3 md:grid-cols-5 xl:grid-cols-7
  ...
  // Show loader only when games are actually loading, not when filtered to empty
  ...
  return (
    <div className="flex flex-col pt-4">
      ...
      <div className="grid grid-cols-1">
        ...
        {!games?.length && (
          <div className="order-3 w-full rounded-lg flex justify-center items-center py-24 px-16 bg-gray-800 md:h-[360px]">
            <div className="flex flex-col gap-2 justify-center items-center w-[218px]">
              ...
              <p className="text-center text-xs xl:text-base">
                No results for{' '}
                <span className="text-neon-1 ">{search}</span>{' '}in{' '}
                <span className="text-neon-1 ">{currentCategory}</span>{' '}
