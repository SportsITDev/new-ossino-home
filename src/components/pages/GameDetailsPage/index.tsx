...
const GameDetails = () => {
  ...
  const normalizedName = name
    ? decodeURIComponent(name).replace(/-/g, ' ').toLowerCase()
    : '';
  const game = games?.find(
    (g) =>
      (g.name && g.name.toLowerCase() === normalizedName) ||
      (g.title && g.title.toLowerCase() === normalizedName),
  );
    ...
    if (balance !== undefined && balance < 500) {
      ...
      dispatch(
        toggleFavorite({
          ...
          aggregator: game?.aggregator_type?.charAt(0) || '',
          ...
          <div className="flex gap-2">
            {!isLargeScreen && (
              <Button
                ...
                className="bg-gray-800 w-8 p-2 h-full flex items-center justify-center"
              >
                <Icon
                  id="statusUpIcon"
                  href={status}
                  className="w-4 h-4 text-neon-1"
                />
              </Button>
            )}
            {typeof LocalStorageHelper.getItem(STORAGE_KEYS.accessToken) === 'string' && (
              isLoadingFavorite ? (
                <span className="w-8 h-8 flex items-center justify-center">
                  <svg className="animate-spin h-4 w-4 text-neon-1" viewBox="0 0 24 24">
                ...
                <button
                  ...
                  className={cn(
                    'z-[60] cursor-pointer w-8 p-2 h-full flex items-center justify-center bg-gray-800 rounded-full',
                  )}
                >
                  <span>
                    <svg
