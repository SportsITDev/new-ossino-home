...
import { formatDate, formatTransactionsDateToRender } from "helpers/transactions/formatDate";
...
const CasinoBetItem = ({ casinoBet }: CasinoBetItemProps) => {
    ...
    const date = { day: formatDate(new Date(placedDate)), time: new Date(placedDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }
    ...
    const getAmount = (item: any): number => {
        if (item.stake > 0 && item.betTransactionType !== "WIN_REQUEST") {
            return Number(item.stake);
        }
        if (item.payout > 0 && item.betTransactionType === "WIN_REQUEST") {
            return Number(item.payout);
        }
        return 0;
    };
            ...
            <div className="min-w-[300px]  lg:w-[450px] text-xs lg:text-sm leading-[18px]">
                <div className="flex flex-row items-center align-items-center gap-1">
                    ...
                    <Tooltip tooltipText={roundId}>
                        <div className=" flex flex-row items-center align-items-center gap-1">
                            <div className="min-w-[230px] text-xs overflow-hidden whitespace-nowrap text-ellipsis cursor-pointer hover:opacity-65" onClick={() => openModal(roundId ?? "", gameName ?? "")}>{TruncateText(roundId ?? "", 35)}</div>
            ...
            <div className="flex min-w-[48px] lg:w-[100px] items-center justify-center ml-auto">
                {isDesktopScreen ? (
                    <div className={cn("rounded-full py-1 px-3 md:text-xs lg:text-xs leading-4",
                        { "bg-success-green-bg text-success-green": betStatus === BET_STATUS.Win || betStatus === BET_STATUS.Won },
                        { "bg-pending-yellow-bg text-pending-yellow": (betStatus === BET_STATUS.Return || betStatus === BET_STATUS.Running) },
                        { "bg-failed-red-bg text-failed-red": betStatus === BET_STATUS.Lost }
                    )}>
                        {betStatus}
                    </div>
                )
