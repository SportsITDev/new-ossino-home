...
const EditAvatarCard = () => {
  ...
  return (
    <div
      className="bg-gray-725 rounded-lg p-4 cursor-pointer"
      onClick={handleClick}
    >
      <div className="flex items-center gap-3">
        <div className="w-12 h-12 rounded-full bg-[#454545] flex items-center justify-center relative overflow-hidden">
          {profileImage ? (
            <img
              src={profileImage}
              alt="avatar"
        ...
        <div className="grow">
          <h3 className="text-lg font-bold leading-none mb-2">{displayName}</h3>
          <p className="text-sm leading-none text-gray-300 flex items-center gap-1">
            <Icon href={levelAvatar} id="levelAvatarIcon" className="w-4 h-4" />
            {level}
          </p>
        </div>
        <Icon href={edit} id="editIcon" className="w-4 h-4" />
      </div>
    </div>
  );
};
