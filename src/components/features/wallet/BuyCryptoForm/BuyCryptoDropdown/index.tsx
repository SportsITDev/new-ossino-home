  ...
  return (
    <div className='flex flex-col px-4 py-1 w-full'>
      {userBalances && Object.values(userBalances).map((currency) => (
        <div
          className='w-full py-3 flex flex-row justify-between border-b border-greyborder last:border-none'
        ...
        >
          <div className='flex justify-start items-center flex-row gap-1.5'>
            ...
            <span className={cn('text-xs leading-4', { 'text-neon': selectedCurrency?.name === currency.name })}>{`${currency.name} (${currency.contraction})`}</span>
          </div>
        </div>
      ))}
    </div>
