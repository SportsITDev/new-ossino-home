  ...
  return (
    <div className="flex flex-col px-4 py-1 w-full">
      {options.map((currency, index) => (
        <div
          key={index}
          className="w-full py-3 flex flex-row justify-between border-b border-greyborder last:border-none cursor-pointer"
        ...
        >
          <div className="flex justify-start items-center flex-row gap-1.5">
            ...
            <span
              className={cn('text-xs leading-4', {
                'text-neon': selectedCurrency?.name === currency.name,
              })}
          ...
          >
            {formatNumber(+(currency.balance ?? 0), 6)}
          </span>
        </div>
      ))}
    </div>
