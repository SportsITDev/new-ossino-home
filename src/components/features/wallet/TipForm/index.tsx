    ...
    <div className="flex flex-col gap-14 md:gap-6">
      <Form {...tipForm}>
        <form className="flex flex-col">
                  ...
                  >
                    <div className="ml-1.5 flex w-full items-center justify-between gap-1.5">
                      ...
                      <span className="flex-1 text-right text-xs leading-4">
                        {formatNumber(+currency.balance, 6)}
                      </span>
                    </div>
                  </Select>
                </FormControl>
          ...
          <Button
            size="lg"
            className={`!w-full mt-8 rounded-lg ${isFormValid ? 'bg-neon-1 text-black hover:bg-neon-2' : 'bg-gray-500 text-gray-300'}`}
      ...
      <TipDialog
        ...
        amount={formatNumber(+tipForm.getValues('amount').replace(/,/g, ''))}
      />
    </div>
  );
};
