...
const PromotionDetails = ({ cmsData }: PromotionDetailsProps) => {
  ...
  if (!cmsData) {
    return (
      <div className="flex items-center justify-center h-32">
        <p className="text-gray-400">No promotion details available</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="mb-4">
        {cmsData.title && (
          <h2 className="text-xl font-bold text-white mb-2">{cmsData.title}</h2>
        )}
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {cmsData.categories.split(',').map((category, index) => (
              <span
                ...
                className="text-xs px-2 py-1 bg-gray-700 rounded-lg text-gray-300"
              >
                {category.trim()}
              </span>
            ))}
      ...
      {cmsData.offerdetails && (
        <div className="prose prose-invert max-w-none">
          <div
            className={cn(
              'markdown text-sm text-gray-300',
