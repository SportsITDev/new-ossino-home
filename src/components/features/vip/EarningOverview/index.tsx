...
const OverviewItem = ({ option, value }: OverviewItemProps) => {
  return (
    <div
      ...
      className="px-4 border border-gray-700 rounded-lg flex gap-1 h-11 items-center"
    >
      {option.icon}
      <span className="text-sm text-gray-200">{option.label}</span>
      <span className="ml-auto font-medium text-neon-1 text-sm">
        {formatNumber(value)}
      </span>
    </div>
  );
...
const EarningOverview = () => {
  ...
  return (
    <div className="rounded-xl bg-gray-800 px-4">
      <Accordion
        type="single"
        collapsible
      >
          ...
          <AccordionContent className="flex flex-col gap-1 md:gap-3 pb-4 md:pb-[38px] xl:pb-4">
            {summaryLoading && (
              <div className="text-center py-8">
                <div className="animate-spin h-8 w-8 border-2 border-neon-1 border-t-transparent rounded-full mx-auto mb-2" />
