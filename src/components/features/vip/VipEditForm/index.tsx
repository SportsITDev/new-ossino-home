...
const VipEditForm = ({ defaultValues, isNew = false }: VipEditFormProps) => {
  ...
  console.log('VipEditForm: Games state', {
  ...
  useEffect(() => {
    console.log('VipEditForm: Loading main games and categories');
    dispatch(fetchGames());
    ...
    if (
      ...
      console.log('VipEditForm: Loading VIP games data (not fresh)');
      dispatch(
        getVipGamesData({
  ...
  useEffect(() => {
    console.log('VipEditForm: Form data update effect', {
      isNew,
    ...
    if (!isNew && selectedGames && selectedGames.length > 0 && games) {
      ...
      console.log('VipEditForm: Mapped selected games', selectedGameIds);
  ...
  const onSubmit = async (values: Values) => {
    ...
    try {
      ...
      if (isNew && !affiliateId && !affiliateLoading) {
        console.log('VipEditForm: Generating new affiliate for VIP creation');
        ...
        console.log(
          'VipEditForm: Affiliate generation already in progress, waiting...',
        );
        return;
      }
      ...
      console.log(
        'VIP games updated successfully, clearing cache and refreshing games data...',
      );
      ...
      if (cache && userData?.id) {
        try {
          ...
          if (parsedCache[userData.id.toString()]) {
            console.log(
              'Clearing localStorage VIP cache for user:',
              userData.id,
            );
      ...
      if (isNew) {
        ...
        console.log('VIP update successful, forcing fresh VIP data check...');
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)}>
        <div className="flex flex-col gap-5 md:gap-4 md:flex-row md:mb-8 xl:gap-5 xl:flex-col xl:mb-10">
          <div className="flex flex-col gap-3 md:w-1/2 justify-between xl:w-full">
            <div>
              ...
              <p className="text-xs text-white">
                ...
                <span className="text-neon-1">20%</span>&nbsp;on every game that
                    ...
                    <FormControl>
                      <div className="relative">
                        <Input
                          ...
                          className="bg-gray-700 border border-gray-600 text-gray-300 cursor-not-allowed pr-10"
                          {...field}
                          readOnly={true}
                          disabled={true}
                        />
          ...
          <div className="flex flex-col gap-3 md:w-1/2 justify-between xl:w-full">
            <div>
              <h2 className="font-bold text-white mb-3">Select Games</h2>
              <p className="text-xs text-white">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {form.watch('games')?.map((gameId) => {
                      ...
                      return game ? (
                        <div
                          key={gameId}
                          className="flex items-center gap-2 bg-neon-1/20 text-neon-1 px-2 py-1 rounded text-xs border border-neon-1/30"
                        >
                          ...
                          <button
                            ...
                            className="text-neon-1 hover:text-red-400 transition-colors"
                          >
                            ×
                          </button>
                        </div>
                            ...
                            <div className="bg-gray-700 rounded-lg border border-gray-600">
                              <div className="p-4">
                                <div className="text-sm text-gray-300 mb-3">
                                  ...
                                  available
                                  {selectedCategory !== 'all' && (
                                    <span className="ml-2 text-neon-1">
                                      in{' '}
                                      {
                                        categories?.find(
                                          (c) =>
                            ...
                            <div className="text-center py-8 text-gray-400 bg-gray-700 rounded-lg border border-gray-600">
                              ...
                              {search && (
                                <button
                                  type="button"
                                  onClick={() => setSearch('')}
                                  className="text-neon-1 text-xs mt-2 hover:underline"
                                >
                                  Clear search
                                </button>
                              )}
        ...
        {vipSuccessMessage && (
          <div className="text-green-500 text-sm mb-4 p-3 bg-green-100 rounded-lg">
            {vipSuccessMessage}
          </div>
        )}
