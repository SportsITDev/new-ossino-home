...
const VipDetailsEnhanced = ({ onViewChange, games, data }: VipDetailsProps) => {
  ...
  useEffect(() => {
    if (
      ...
      console.log(
        'VipDetailsEnhanced: Loading VIP games data for existing affiliate (data not fresh):',
        affiliate.data?.btag,
      );
      dispatch(
        getVipGamesData({
  ...
  const onCopy = async () => {
    const siteUrl =
      import.meta.env.VITE_WEBSITE_URL || 'https://ossino-stg.negroup-tech.net';
    const url =
  ...
  const onShare = async () => {
    ...
    console.log('VipDetailsEnhanced: Sharing URL', {
      url,
      siteUrl,
      btag: affiliate.data?.btag,
    });
  ...
  if (affiliate.loading) {
    return (
      <div className="px-4 py-8 text-center">
        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-neon-1 mx-auto mb-4" />
        <p className="text-gray-300">Setting up your VIP account...</p>
      </div>
    );
  }
  ...
  if (hasAffiliateData && selectedGames.loading) {
    return (
      <div className="px-4 py-8 text-center">
        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-neon-1 mx-auto mb-4" />
        <p className="text-gray-300">Loading your VIP data...</p>
      </div>
    );
  }
  ...
  // Main VIP details view - always use dynamic affiliate URL
  const siteUrl =
    import.meta.env.VITE_WEBSITE_URL || 'https://ossino-stg.negroup-tech.net';
  const vipUrl =
    hasAffiliateData && affiliate.data?.btag
  ...
  console.log('VipDetailsEnhanced: VIP URL generated', {
    vipUrl,
    siteUrl,
    btag: affiliate.data?.btag,
    hasAffiliateData,
  ...
  return (
    <div className="px-4 flex flex-col md:flex-row gap-5 md:gap-4 xl:flex-col xl:gap-5 xl:p-0">
      <div className="p-4 rounded-xl bg-gray-800 flex-1">
        <div className="flex justify-between mb-3">
          <h2 className="font-bold">Your VIP Page</h2>
          <Link
            className="text-neon-1 flex gap-1 items-center text-sm font-medium xl:hidden"
            to="edit"
          >
            Edit
            <Icon id="editIcon" href={edit} className="h-4 w-4 text-neon-1" />
          </Link>
          <Button
            variant="outline"
        ...
        {vipName && (
          <div className="mb-3">
            <p className="text-sm text-gray-300">
              VIP Name:{' '}
              <span className="text-white font-medium">{vipName}</span>
            </p>
          </div>
        )}
        ...
        <p className="text-xs text-white mb-3">
          ...
          <span className="text-neon-1">{commissionPercentage}%</span>&nbsp;on every game that the
          players play after registering through your VIP page.
        </p>

        {vipUrl && (
          <div
            className="px-3 h-10 rounded-lg border border-gray-600 mb-3 flex justify-between items-center cursor-pointer"
            onClick={onCopy}
          >
            <span className="text-gray-100 text-sm truncate flex-1 mr-2">
              {vipUrl}
            </span>
            <div className="flex items-center gap-2">
              {copiedText && (
                <span className="text-neon-1 text-xs">{copiedText}</span>
              )}
              <Icon id="copyIcon" href={copy} className="h-4 w-4" />
            </div>
          </div>
        )}
      ...
      <div className="flex flex-col gap-5 flex-1 md:gap-3">
        ...
        <Button
          size="xl"
          className="bg-gray-800 rounded-xl text-white text-base font-bold w-full justify-between hidden xl:flex"
