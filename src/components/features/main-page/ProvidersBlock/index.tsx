...
const ProvidersBlock = () => {
  ...
  const filteredProviders = providers?.filter((provider) => {
    ...
    return (
      providerCode &&
      games?.some((game) => game.providers.includes(providerCode))
    );
  });
    ...
    return (
      <Slider label="Providers" withShadow>
        {Array.from({ length: 5 }).map((_, index) => (
          ...
          >
            <Skeleton className="bg-gray-600 min-w-[180px] h-20 md:h-24 flex items-center justify-center rounded-[12px] cursor-pointer" />
          </SwiperSlide>
        ))}
      </Slider>
    );
