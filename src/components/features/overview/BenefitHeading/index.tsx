...
const BenefitHeading = ({
  ...
  return (
    <div className="flex items-center gap-3 w-full">
      <div
        className={cn(
          'w-14 h-14 bg-gray-700 border border-gray-700 rounded-xl flex items-center justify-center benefit-image transition-all duration-200',
          {
      ...
      <div className="flex flex-col gap-2 items-start grow">
        <div className="flex items-center gap-2">
          ...
          {isCurrentLevel && (
            <span className="text-xs bg-gradient-to-r from-amber-400 to-amber-600 text-black px-2 py-0.5 rounded-full font-medium">
              Current
            </span>
          )}
        </div>
        <span
          className={cn(
            'inline-flex text-gray-100 font-medium items-center justify-center h-[18px] px-2 rounded-xl text-[10px] transition-colors duration-200',
            {
              'bg-gradient-to-r from-emerald-500 to-emerald-600 text-white shadow-sm':
                isOpened,
              'bg-gray-700': !isOpened,
