...
const DetailsView = ({
  ...
  useEffect(() => {
    console.log('VipSheet DetailsView: Effect triggered', {
      userData: !!userData,
      affiliateData: !!affiliateData,
      affiliateBtag: affiliateData?.btag,
      gamesData: !!games,
      gamesLoading,
      vipGamesAreFresh,
    });
    ...
    if (
      ...
      console.log(
        'VipSheet: Fetching VIP games (data not fresh or missing) for user:',
        userData.id,
        'affiliate:',
        affiliateData.btag,
      );
  ...
  if (gamesLoading) {
    return (
      <div className="flex items-center justify-center p-8">
        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-neon-1"></div>
        <span className="ml-3 text-white">Loading VIP data...</span>
      </div>
    );
  }

  console.log('VipSheet DetailsView: Rendering VipDetailsEnhanced', {
    data: !!data,
      ...
      <SheetContent className="z-[999] !max-w-[600px] w-full">
        ...
        <div className="mb-5 flex justify-between items-center">
          <div className="flex items-center gap-2">
            <SheetClose className="w-8 h-8 border border-gray-700 rounded-lg flex items-center justify-center">
              <span className="sr-only">Close</span>
              <ArrowRight2Icon />
            </SheetClose>
            <div className="bg-gray-800 px-4 h-8 flex items-center justify-center rounded-lg">
              <span className="font-medium text-neon-2 text-sm">
