...
const formatMentionText = (text: string) => {
  ...
  if (!tags) {
    return <span className="leading-[20px] text-gray-200">{text}</span>;
  }

  const usernames = tags.map((tag) => tag.split(reg)[1]);
  return (
    <span className="leading-[20px] text-gray-200">
      {text.split(reg).reduce(
        (prev, current) => {
          const n = [...prev];
          n.push(
            usernames.includes(current) ? (
              <span key={current} className="text-neon-1">
                @{current}
              </span>
            ) : (
              current
      ...
      <div className="flex flex-row">
        <span className="flex flex-row gap-1 items-start">
          ...
          <span className="text-sm font-regular leading-tight">
            <span className="font-bold leading-4 text-white">{nickName}</span>
            <span>{`: `}</span>
    ...
    <div className="bg-gray-800 rounded-lg p-2 w-full">
      <span className="flex flex-row gap-1 items-start mb-2">
        ...
        <span className="text-sm font-regular leading-tight flex items-center gap-1">
          <span className="font-bold leading-4 text-white">{sender}:</span>
          <span className="text-gray-200">Tipped</span>
          <span className="leading-[20px] font-medium text-neon-1">
            @{receiver}
          </span>
        </span>
      </span>
      <button
        onClick={handleClick}
        className="bg-gray-700 rounded-md p-2 flex items-center gap-2 w-full"
        type="button"
      >
