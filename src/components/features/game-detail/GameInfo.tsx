...
const GameInfo = ({ game }: GameInfoProps) => {
  ...
  return (
    <div className="flex z-10 flex-col w-full gap-5 md:bg-dark-green md:p-5 rounded-xl order-2 md:order-1">
      {/* Filters */}
      <div className="flex flex-col gap-3">
        ...
        <Slider className="overflow-hidden">
          {filters.map((item) => (
            ...
            >
              <div className="bg-gray-700 px-3 py-2 text-center text-xs rounded-lg cursor-pointer">
                {item}
              </div>
            </SwiperSlide>
          ))}
      ...
      {/* Currency Selector */}
      <div className="flex flex-col bg-gray-800 md:bg-transparent px-4 py-4 md:py-0 md:px-0 rounded-xl gap-4">
        {hasCurrencies && currentCurrency && currentEntity && (
          <div className="flex flex-col gap-1">
            ...
            <div className="relative w-full sm:w-auto max-w-full">
              ...
              <button
                className="flex items-center justify-between h-10 px-3 w-full sm:min-w-[340px] sm:max-w-[400px] max-w-full bg-gray-700 rounded-lg border border-gray-600 hover:border-gray-500 transition-colors"
              ...
              >
                <span className="flex items-center gap-1.5">
                  ...
                  <span className="ml-2 text-xs text-neon-1">
              ...
              {isDropdownOpen && (
                <div className="absolute top-full left-0 mt-1 w-full sm:min-w-[340px] sm:max-w-[400px] max-w-full bg-gray-700 rounded-lg border border-gray-600 shadow-lg z-20 max-h-60 overflow-y-auto">
                  {currencies.map((currency) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          'flex items-center gap-2 px-3 py-2 w-full text-left hover:bg-gray-600 transition-colors',
                      ...
                      >
                        ...
                        <span className="ml-auto text-xs text-neon-1">
        ...
        <div className="flex justify-between gap-2">
          {hasCurrencies && currentCurrency && (
            <Button
              className="text-gray-900 flex w-full gap-1 justify-center items-center"
          ...
          <Button
            className="bg-transparent border border-neon-1 text-neon-1 flex w-full gap-1 justify-center items-center"
            onClick={handleFunPlay}
          >
            <Icon id="happyEmojiIcon" href={happy} className="w-4 h-4" />
            Fun Play
          </Button>
        </div>
      </div>
